...
> **⚠️ v1.6.2 勘误注记（2026-09-25）**：本节下方的 schema 是**起草期快照**，与仓库实际实现已不一致，
> 请勿作为权威依据。实际差异：① 字段集已扩展（见 §16.2 与 v1.6.2 附录）；② `"total": 500` 与各文件
> 篇幅估计（a1 ~500 词 / ~25KB 等）**均与实测不符** —— 实测 `total` 为 942 / 898 / 802 / 1428（英语），
> 文件体积 137.5 / 167.9 / 223.8 / 354.2 KB。**权威口径见 `docs/spec/v1.6.2/main.md` 附录 A。**
> 本节保留原文仅为记录起草意图。
...
> **⚠️ v1.6.2 勘误注记（2026-09-25）**：本节的 `"total": 80` 是 **v2 升级前的占位值**，早已失效
> （实测英语 942/898/802/1428）。下方 `WordlistEntry` 接口也**不完整** —— 实际实现还含
> `frequency`、`semanticConflicts`（v1.6.0）与 `example` / `exampleTranslation` / `exampleSource`（v1.6.2）。
> 另：v1.6.1 起除 `lemma`/`pos`/`translation`/`cefr` 外的字段**全部为可选**，而本节写作时是必填口径。
> **权威口径见 `docs/spec/v1.6.2/main.md` 附录 A。**
