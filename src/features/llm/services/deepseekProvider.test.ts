/**
 * deepseekGenerate 单元测试 (v1.4.0 Stage 1 — T01..T02)
 *
 * 覆盖 SPEC 要求 2 个 case:
 * - T01 [critical]: POST 到 VITE_LLM_PROXY_URL, request body 包含 provider=deepseek / model=deepseek-chat / expectedLanguage
 * - T02 [critical]: 5xx / 4xx 响应时, 抛带 status 的 Error
 *
 * 设计:
...
describe('deepseekGenerate (v1.4.0 Stage 1 — T01..T02)', () => {
  // 测试涉及的 env 字段
  const ENV_KEYS = [
    'VITE_LLM_PROVIDER',
    'VITE_LLM_PROXY_URL',
  ...
  it('T01 [critical]: POST 到 VITE_LLM_PROXY_URL, body 包含 deepseek schema', async () => {
    ...
    // body 字符串包含 provider=deepseek + model=deepseek-chat + expectedLanguage=en
    const bodyStr = String(calledInit.body);
    expect(bodyStr).toContain('"provider":"deepseek"');
    expect(bodyStr).toContain('"model":"deepseek-chat"');
  ...
  it('T02 [critical]: 5xx 响应时, 抛带 status 的 Error', async () => {
    ...
    try {
      await deepseekGenerate({ prompt: 'test' });
    } catch (e) {
      caught = e as Error;
    }
