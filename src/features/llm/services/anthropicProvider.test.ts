/**
 * anthropicGenerate 单元测试 (v1.4.0 Stage 2 — T01..T03)
 *
 * 覆盖 SPEC 要求 3 个 case:
 * - T01 [critical]: POST 到 VITE_LLM_PROXY_URL, request body 包含 provider=anthropic / model=claude-3-5-haiku-20241022 / expectedLanguage
 ...
 * - 模式与 deepseekGenerate / openaiGenerate 测试完全镜像
 */
...
describe('anthropicGenerate (v1.4.0 Stage 2 — T01..T03)', () => {
  // 测试涉及的 env 字段
  const ENV_KEYS = [
    'VITE_LLM_PROVIDER',
    'VITE_LLM_PROXY_URL',
  ...
  it('T01 [critical]: POST 到 VITE_LLM_PROXY_URL, body 包含 anthropic schema', async () => {
    ...
    // body 字符串包含 provider=anthropic + model=claude-3-5-haiku-20241022 + expectedLanguage=en
    const bodyStr = String(calledInit.body);
    expect(bodyStr).toContain('"provider":"anthropic"');
    expect(bodyStr).toContain('"model":"claude-3-5-haiku-20241022"');
  ...
  it('T03 [critical]: 5xx 响应时, 抛带 status 的 Error', async () => {
    ...
    try {
      await anthropicGenerate({ prompt: 'test' });
    } catch (e) {
      caught = e as Error;
    }
