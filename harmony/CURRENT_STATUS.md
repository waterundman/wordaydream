...
| Hvigor | `6.22.7` |
| ohpm | `6.0.1` |
| 应用版本 | 单一版本真源（仓库根包 `3.4.0=harmony major+2`）；AppScope `1.4.0`、entry `1.4.0` 与之对齐，由 `scripts/check-version-alignment.mjs` 自动校验；v3.0.0 起 harmony 跨入 major 1.0.0，versionCode 线性规则（major≠0 未覆盖）自动跳过校验，数值延续递增 `1000055` |
